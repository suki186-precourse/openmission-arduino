...
[ ] 조이스틱 쉴드 핀맵 확인 (A0: X축, D8: 버튼)
[ ] I2C OLED 핀 연결 (A4: SDA, A5: SCL)
[ ] 3색 LED 핀 연결 (D3: R, D4: G, D5: B)
[ ] 부저 핀 연결 (D2)
[ ] setup() 함수에서 각 핀 모드(INPUT_PULLUP, OUTPUT) 설정
[ ] I2C OLED 라이브러리 초기화

**🔹 2. 입력 처리**

[ ] 조이스틱 X축 값 읽기
[ ] 읽어온 값을 패들 X좌표(0~127)로 변환
[ ] 조이스틱 버튼(D8) 값 읽기 (PULLUP)
[ ] 버튼 입력 시 디바운스 처리
...
[ ] 공이 패들 위에 붙어서 함께 이동
[ ] 버튼 입력 시 공 발사 및 PLAYING 상태로 변경

[Playing 상태]

[ ] 공의 위치(x, y)와 속도(dx, dy)를 기반으로 이동 처리
[ ] 공-벽(상, 좌, 우) 충돌 검사 및 방향 반전
[ ] 공-패들 충돌 검사 및 방향 반전
[ ] 공-바닥 충돌 검사 (생명 감소, READY 상태로 복귀)
[ ] 벽돌 배열 생성 및 관리 (예: 2차원 배열)
[ ] 공-벽돌 충돌 검사 (벽돌 삭제, 공 방향 반전)

[GameOver 상태]

[ ] 생명 0개 시 GAME_OVER 상태로 변경
